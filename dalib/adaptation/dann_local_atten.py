...
from common.modules.classifier import Classifier_multi_head as MultiClassifierBase
...
class DomainAdversarialLoss(nn.Module):
    ...
    def forward(self, f_s: torch.Tensor, f_t: torch.Tensor,
        ...
        loss = 0.5 * ( self.bce(d_s, d_label_s, w_s.view_as(d_s)) + self.bce(d_t, d_label_t, w_t.view_as(d_t)))
        return loss

class LocalDomainAdversarialLoss(nn.Module):
    r"""
    The Domain Adversarial Loss proposed in
    `Domain-Adversarial Training of Neural Networks (ICML 2015) <https://arxiv.org/abs/1505.07818>`_
    ...
    def __init__(self, domain_discriminator: nn.Module, reduction: Optional[str] = 'mean',
                 grl: Optional = None):
        super(LocalDomainAdversarialLoss, self).__init__()
        self.grl = WarmStartGradientReverseLayer(alpha=1., lo=0., hi=1., max_iters=1000, auto_step=True) if grl is None else grl
        self.domain_discriminator = domain_discriminator
    ...
    def forward(self, f_s: torch.Tensor, f_t: torch.Tensor,
        ...
        d_label_s = torch.ones((f_s.size(0),1,f_s.size(2),f_s.size(3))).to(f_s.device)
        d_label_t = torch.zeros((f_t.size(0), 1,f_t.size(2),f_t.size(3))).to(f_t.device)
        ...
        loss = 0.1 * ( self.bce(d_s, d_label_s, w_s.view_as(d_s)) + self.bce(d_t, d_label_t, w_t.view_as(d_t)))
        return loss,  d
...
class MultiImageClassifier(MultiClassifierBase):
    def __init__(self, backbone: nn.Module, num_classes: int, bottleneck_dim: Optional[int] = 256, **kwargs):
        bottleneck = nn.Sequential(
            # nn.AdaptiveAvgPool2d(output_size=(1, 1)),
            # nn.Flatten(),
        ...
        super(MultiImageClassifier, self).__init__(backbone, num_classes, bottleneck, bottleneck_dim, **kwargs)